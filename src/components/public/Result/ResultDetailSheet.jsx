...
async function loadCanvasFonts() {
  ...
  const font = new FontFace(
    "Metrophobic",
    "url(/fonts/Metrophobic-Regular.ttf)"
  );
...
function drawWinnersOnCtx(ctx, w, entries, canvasWidth) {
  ...
  sorted.forEach((winner, i) => {
    ...
    // Participant name
    ctx.save();
    ctx.font = `bold ${w.fontSize || 28}px "Metrophobic", sans-serif`;
    ...
    if (teamName) {
      ctx.save();
      ctx.font = `normal ${w.teamFontSize || 20}px "Metrophobic", sans-serif`;
...
export async function generatePosterOnCanvas(template, result) {
  await loadCanvasFonts(); // ← add this
  ...
  if (rn) {
    ctx.save();
    ctx.font = `${rn.fontWeight || "normal"} ${rn.fontSize || 40}px "Metrophobic", "Segoe UI", system-ui`;
  ...
  if (cn) {
    ctx.save();
    ctx.font = `${cn.fontWeight || "normal"} ${cn.fontSize || 32}px "Metrophobic", "Segoe UI", system-ui`;
  ...
  if (gr && result.group) {
    ctx.save();
    ctx.font = `${gr.fontWeight || "normal"} ${gr.fontSize || 24}px "Metrophobic", "Segoe UI", system-ui`;
    ctx.fillStyle = gr.color || "#ffffff";
    ctx.textAlign = gr.textAlign || "left";
    ctx.textBaseline = "alphabetic";
    ctx.fillText(result.group, resolveX(gr.textAlign, gr.x, W), gr.y);
    ctx.restore();
  }
...
export default function ResultDetailSheet({ result, isOpen, onClose }) {
  ...
  const hasServerImage =
    result?.resultUrl && result.resultUrl.trim() !== "" && !imageError;
    ...
    try {
      if (navigator.share) {
        await navigator.share({
          title: `${result.categoryName} - Results`,
          text,
  ...
  const handleShareGenerated = async () => {
    ...
    try {
      ...
      const file = new File([blob], `${result.categoryName}_result.png`, {
        type: "image/png",
      });
      ...
      {isOpen && (
        <>
          ...
          >
            {/* Header */}
            <div
              className="sticky z-50 top-0 bg-[#0f0f0f]/95 backdrop-blur-sm
                  ...
                  >
                    {!imageError ? (
                      <Image
                        ...
                        unoptimized={result.resultUrl.includes(
                          "cloudinary.com",
                        )}
                        loading="lazy"
                ...
                <div className="space-y-4">
                  ...
                  {generating && (
                    ...
                    >
                      <Loader2
                        size={18}
                        className="animate-spin text-amber-400/60"
                      />
                      <span className="text-xs text-white/35">
                        Generating poster…
                      </span>
                ...
                <div className="space-y-3">
                                ...
                                <p className="text-[11px] text-white/40 truncate mt-0.5">
                                  {entry.teamId.color && (
                                    <span
                                      ...
                                      style={{
                                        backgroundColor: entry.teamId.color,
                                      }}
                                    />
                                  )}
                                  {entry.teamId.name}
                  ...
                  {result.entries?.length > 4 && (
                    <p className="text-center text-[10px] text-white/25 pt-2">
                      +{result.entries.length - 4} more participant
                      {result.entries.length - 4 !== 1 ? "s" : ""}
                    </p>
                  )}
                </div>
              )}
